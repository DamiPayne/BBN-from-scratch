...
#How to make a artificaial neural netwrok from scratch :)
...
n_hidden = 20 
#No. of varribles being input
n_in = 20
#No. of varribles being output
n_out = 20
#No. samples
n_sample = 500
...
#So the neural network cycles to aviod local minima and find the global minimum
#Cycles are also known as Epochs
#Momenttum controls the speed at which the NN forms a solution
#Momentum helps the network to overcome obstacles (local minima) in the error surface
#Learning rate is the the rate of convergence between the current solution and the global minimum
learning_rate = 0.01
momentum = 0.9
...
def sigmoid(x):
	'''our sigmiod function turns numbers into probabilities'''		 	
...
# bV and bW represent ajustments for bais
def train(x, t, V, W, bV, bW):
	...
	#Then creating a new set of wieghts in the opposite direction
	#Backward propagation
	#It is essecially our matrix of values flipped backwards Y-t
	Ew = Y - t
	...
	#However for classification the cross entry function offers the best preformance
